        ...
        <div class="card card-panel">
            <div class="card-header">
                <h6 class="card-panel__title mb-0">
                    <i class="bi bi-pencil-fill me-2 text-warning"></i>Edit: {{ $motor->motor_code }}
                </h6>
            </div>
            <div class="card-body">
                <form method="POST" action="{{ route('motors.update', $motor) }}" novalidate>
                    @csrf @method('PUT')
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" id="motor_code" name="motor_code"
                                ...
                                value="{{ old('motor_code', $motor->motor_code) }}" required>
                        ...
                        <div class="col-md-6">
                            ...
                            <select id="location" name="location" class="form-select @error('location') is-invalid @enderror" required>
                                ...
                                <option value="MA-1" {{ old('location', $motor->location) == 'MA-1' ? 'selected' : '' }}>MA-1</option>
                                <option value="MA-2" {{ old('location', $motor->location) == 'MA-2' ? 'selected' : '' }}>MA-2</option>
                                <option value="MA-3" {{ old('location', $motor->location) == 'MA-3' ? 'selected' : '' }}>MA-3</option>
                                <option value="MA-4" {{ old('location', $motor->location) == 'MA-4' ? 'selected' : '' }}>MA-4</option>
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <select id="category" name="category" class="form-select @error('category') is-invalid @enderror" required>
                                ...
                                <option value="<100" {{ old('category', $motor->category) == '<100' ? 'selected' : '' }}><100 HP</option>
                                <option value="100-500" {{ old('category', $motor->category) == '100-500' ? 'selected' : '' }}>100-500 HP</option>
                                <option value=">500" {{ old('category', $motor->category) == '>500' ? 'selected' : '' }}>>500 HP</option>
                            </select>
                    ...
                    <div class="d-flex gap-2 mt-4">
                        <button type="submit" class="btn btn-warning text-dark fw-semibold">
                            <i class="bi bi-check-lg me-1"></i>Update Motor
                        </button>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const oldArea = "{{ old('area', $motor->area ?? '') }}";
